...
private:
   ...
   {
//      do{
fprintf(stderr, "..1.. Pos=%d, MaxLen=%d, buf=%s\n", Pos_, MaxLen_, buf_);
...
// <-- 20140801 modify by k288 for 接收異常封包資料造成 core dump 的情況
fprintf(stderr, "..2.. rdsz=%d, Pos=%d, buf=%s\n", rdsz, Pos_,  buf_); 
         if (rdsz>0) {
            Pos_  += rdsz;
            buf_[Pos_] = 0;
         } else {
...
fprintf(stderr, "..2-1.. Seperator=%s, Pos=%d, buf=%s\n", Seperator_, Pos_,  buf_);
         while ((S = strchr(buf_, Seperator_))) {
            int len = S - buf_;
            *S      = 0;
fprintf(stderr, "..3.. len=%d, Pos=%d, S=%s, buf=%s\n", len, Pos_, S, buf_);
            if (len>50)
               OnPktRecv(buf_, len);
            Pos_   -= len +SepLen_;
fprintf(stderr, "..4.. Pos=%d, buf=%s\n", Pos_, buf_);
            if(Pos_ > 0){
fprintf(stderr, "..4-1.1.. Pos=%d, buf=%s\n", Pos_, buf_);
               memcpy(buf_, buf_+ len + SepLen_, Pos_);
               buf_[Pos_] = 0;
fprintf(stderr, "..4-1.2.. Pos=%d, buf=%s\n", Pos_, buf_);
            }else{
               Pos_ = 0; 
               memset(buf_, 0, sizeof(buf_));
fprintf(stderr, "..4-2.. Pos=%d, buf=%s\n", Pos_, buf_);
               return;
            }
         }
fprintf(stderr, "..5.. Pos=%d, buf=%s\n", Pos_, buf_);
         Pos_ = 0;
         memset(buf_, 0, sizeof(buf_));  
//      }  while (1);
   }
