...
#ifdef K_SOCKET_OVERLAPPED//fonwin:支援Windows的 Overlapped IO
# include "TimerThread.h"
# include "pod_vector.h"
namespace ost {
   class CCXX_CLASS_EXPORT TRecvBuf {//放在 public, BCB4 才能debug
      char*    GetBeg_;
      char*    GetEnd_;
      char*    Buffer_;
      size_t   BufferSize_;
   public:
      inline TRecvBuf() : Buffer_(0), BufferSize_(0), GetBeg_(0), GetEnd_(0) {
      }
      inline ~TRecvBuf() {
         if(Buffer_)
            std::realloc(Buffer_,0);
      }
      inline size_t size() const {
         return GetEnd_ - GetBeg_;
      }
      inline const char* c_str() const {
         return GetBeg_;
      }
      inline void clear() {
         GetBeg_ = GetEnd_ = Buffer_;
      }
      inline void erase_head(size_t count) {
         if((GetBeg_ += count) >= GetEnd_)
            clear();
      }
      inline void append(const void* buf, unsigned len) {
         char*    bufend = Buffer_ + BufferSize_;
         size_t   leftsz = bufend - GetEnd_;
         if(leftsz < len) {
            size_t   headsz = GetBeg_ - Buffer_;
            size_t   cursz  = size();
            if(headsz + leftsz >= len) {
               memcpy(Buffer_, GetBeg_, cursz);
               GetBeg_ = Buffer_;
            } else {
               if((BufferSize_ += (len + 1024)) > 1024*1024)
                  BufferSize_ = ((BufferSize_ / (1024*1024)) + 2) * (1024*1024);
               else
                  BufferSize_ = ((BufferSize_ / (1024*10)) + 2) * (1024*10);
               if(headsz < 1024) {
                  Buffer_ = reinterpret_cast<char*>(std::realloc(Buffer_, BufferSize_));
                  GetBeg_ = Buffer_ + headsz;
               } else {
                  char* newbuf = reinterpret_cast<char*>(std::realloc(0, BufferSize_));
                  memcpy(newbuf, GetBeg_, cursz);
                  std::realloc(Buffer_, 0);
                  GetBeg_ = Buffer_ = newbuf;
               }
            }
            GetEnd_ = GetBeg_ + cursz;
         }
         memcpy(GetEnd_, buf, len);
         GetEnd_ += len;
      }
   };
} // namespace ost
...
public:
	 ...
	 */
	inline int send(const void *buf, sk_size_t len)
	 ...
	 */
	inline int receive(void *buf, sk_size_t len)
	 ...
	 */
	inline int peek(void *buf, sk_size_t len)
...
public:
	 ...
	 * @param len of bytes to send.
	 */
	inline int transmit(const char *buffer, sk_size_t len)
...
*/
const int WM_SET_HWND          = WM_USER+9;
const int WM_USER_SOCKET_EVENT = WM_USER+10;

class SocketServiceSync
{
   HWND  Hwnd_;
public:
   inline SocketServiceSync()
      : Hwnd_(0)
#ifdef K_SOCKET_OVERLAPPED//fonwin:支援Windows的 Overlapped IO
      , Timer_(this)
#endif
      { }
   virtual ~SocketServiceSync();

   virtual void DoSocketEvent(SOCKET, long events) = 0;
   virtual void SelectEvents (SOCKET, long events, SocketPort*);
...
#ifdef K_SOCKET_OVERLAPPED//fonwin:支援Windows的 Overlapped IO
//部 [分斷] 線事件, 必須用[非同步]的方式處理.
private:
   Kway::TTimer<SocketServiceSync>   Timer_;
   class TPendingDisconnects : public Kstl::pod_vector<SocketPort*>, public Mutex {
   };
   TPendingDisconnects  PendingDisconnects_;
   K_mf(void) OnTimer(Kway::TTimerBase*);
public:
   void AddPendingDisconnect(SocketPort* port);
   void DelPendingDisconnect(SocketPort* port);
#endif
};
class TInitWSA_Impl;
...
public:
   TInitWSA();
   ~TInitWSA();
   void AddService (SocketServiceSync*);
   void DelService (SocketServiceSync*);
};
extern TInitWSA   InitWSA;
#endif // WIN32
#ifdef K_SOCKET_OVERLAPPED//fonwin:支援Windows的 Overlapped IO
class TSocketPorts;
class TOvrIOData : public OVERLAPPED
{
   inline  TOvrIOData() {}//建構放在private = 禁止您使用new.
   inline ~TOvrIOData() {}//解構放在private = 禁止您使用delete.

   void Init();
   void FreeInternal();
   friend class TSocketPorts;
public:
   WSABUF   WsaBuf_;
   int      OperationType_; //FD_WRITE, FD_READ
   ...
   ///僅能透過Alloc()建構,取得.
   static TOvrIOData* Alloc();
   ///僅能透過Free()解構.
   void Free();

   ///> return true=success, false=fail
   bool WaitRecv(SOCKET);
   ///> return Send length.
   int WaitSend(SOCKET, const void* buf, int len);
};
#endif
...
//	HANDLE event;  fonwin不用event了.
   ...
   #ifdef WIN32//fonwin:採用WSAAsyncSelect()
      long* eventDoing;
      void updateEvent (bool setEv);
   public:
      void doEvent     (long events);
   #endif
...
#ifdef K_SOCKET_OVERLAPPED//fonwin:支援Windows的 Overlapped IO
public:
	bool isDetectOutput() const
		{ return detect_output; }
private:
   friend class SocketServiceSync;
   friend class TInitWSA_Impl;
   int            OvrHandle_;
   TOvrIOData*    OvrRead_;
   TOvrIOData*    OvrWrite_;
   //std::string    RecvBuf_; 使用 std::string 若超過2M,會有問題!
   TRecvBuf    RecvBuf_;
   unsigned    RecvBufFrom_;
protected:
   int send(const void* buf, int len) {
      if(OvrHandle_==0 || OvrWrite_->BytesTransferred_ < 0)///傳送中.
         return 0;
      return OvrWrite_->WaitSend(so,buf,len);
   }
   int receive(void* buf, int len) {
      return peek3(buf, len, true);
   }
   int peek(void* buf, int len) {
      return peek3(buf, len, false);
   }
private:
   int peek3(void* buf, int len, bool erase) {
      unsigned rsz = (unsigned)(RecvBuf_.size() - RecvBufFrom_);
      if(rsz <= 0)
         return peekOvr(buf, len, erase);
      const char* rbuf = RecvBuf_.c_str() + RecvBufFrom_;
      if(rsz >= (unsigned)len) {
         memcpy(buf, rbuf, len);
         if(erase)
            if(rsz==(unsigned)len) {
      ...
      return rsz + peekOvr(((char*)buf) + rsz, len - rsz, erase);
   }
   int peekOvr(void* buf, int len, bool erase) {
      int blen = OvrRead_->BytesTransferred_;
      if(blen <= 0)
         return 0;
      if((blen -= OvrRead_->BytesPoped_) <= 0)
         return 0;
      if(len > blen)
         len = blen;
      memcpy(buf, OvrRead_->WsaBuf_.buf + OvrRead_->BytesPoped_, len);
      if(erase)
         OvrRead_->BytesPoped_ += len;
      return len;
   }
   void ReadOvr2Buf() {
      int len = OvrRead_->BytesTransferred_;
      if(len <= 0)
         return;
      if((len -= OvrRead_->BytesPoped_) <= 0)
         return;
      if(RecvBufFrom_ > 0) {
         //RecvBuf_.erase(0, RecvBufFrom_);
         RecvBuf_.erase_head(RecvBufFrom_);
         RecvBufFrom_ = 0;
      }
      RecvBuf_.append(OvrRead_->WsaBuf_.buf + OvrRead_->BytesPoped_, len);
      OvrRead_->BytesTransferred_ = OvrRead_->BytesPoped_ = 0;
   }
protected:
#else
	/**
	...
	void endSocket(void);//fonwin

   //fonein:20031117:begin{ 這段程式,unix也必須增加
   //為了讓一般的 Socket 也可以方便的使用 SocketService, 所以增加底下2個members
   inline SocketPort(int domain, int type, int protocol)
	   : Socket(domain, type, protocol),
        next(0), prev(0), service(0),
        #ifdef WIN32
           //event(CreateEvent(NULL,TRUE,FALSE,NULL)),   fonwin:不用 event 了!
        #elif defined(CCXX_USE_POLL)
	        ufd(0),
        #endif
	     detect_pending(false), detect_output(false), detect_disconnect(false)
        #ifdef WIN32
           ,eventDoing(0)
        #endif
        #ifdef K_SOCKET_OVERLAPPED//fonwin:支援Windows的 Overlapped IO
        ...
        #endif
      { }
   inline void setDetectPendingDisconnect (bool p, bool d)
      { if(d==detect_disconnect  &&  p==detect_pending)
           return;
        detect_disconnect = d;
        detect_pending    = !p;//為了讓 setDetectPending(p) 生效
        setDetectPending(p);
      }
   // 20031117:end}
   //fonwin:20031123:begin{ 為了避免accept()失敗,會產生 exception, 造成後續處理上的困擾,
   //                       所以增加底下的建構式(s 若為 INVALID_SOCKET,則仍會產生exception):
   inline SocketPort(SOCKET s)
	   : Socket(s),
        next(0), prev(0), service(0),
        #ifdef WIN32
           //event(CreateEvent(NULL,TRUE,FALSE,NULL)),   fonwin:不用 event 了!
	        ...
	        ufd(0),
        #endif
	     detect_pending(true), detect_output(false), detect_disconnect(true)
        #ifdef WIN32
           ,eventDoing(0)
        #endif
        #ifdef K_SOCKET_OVERLAPPED//fonwin:支援Windows的 Overlapped IO
...
public:
	...
	inline int getCount(void) const
		{return count;};
   #ifdef K_SOCKET_OVERLAPPED//fonwin:支援Windows的 Overlapped IO
      inline void start()  {}//SocketService不用thread.
   #endif
};
