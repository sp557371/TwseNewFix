...
{
   SesMgr_   = (TEvSesMgr*)sm;
// --> 20141022 add by k288 for 填入 MsgTimer_ 預設值
	MsgTimer_ = NULL;
// <-- 20141022 add by k288 for 填入 MsgTimer_ 預設值
}
//---------------------------------------------------------------------------
K_mf(void) TMsgSes::OnPktRecv(void* vbuf, int sz)
{
   if(!ByTelnet_){
      ...
      if(CurSeqNo_ <= LastSeqNo_)
         return;
// --> 20140806 modify by k288 for 接收異常封包資料造成 core dump 的情況
   }else if(MsgTimer_ == NULL){
...
K_mf(void) TMsgSes::OnTimer(TTimerBase*)
{
   MsgTimer_->Stop();
   delete MsgTimer_;
// --> 20140806 add by k288 for 接收異常封包資料造成 core dump 的情況
	MsgTimer_ = NULL;
// <-- 20140806 add by k288 for 接收異常封包資料造成 core dump 的情況
   Close();
}
