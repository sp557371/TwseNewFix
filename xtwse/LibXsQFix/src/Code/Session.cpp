...
Session::Session( Application& application,
                  MessageStore* pMessageStore,
                  const SessionID& sessionID,
                  const DataDictionary& dataDictionary,
                  const SessionTime& sessionTime,
                  int heartBtInt, Log* pLog )
// } cooper
  ...
  try
  {
    ...
    if ( !m_state.receivedLogon() )
    {
      ...
      else if ( m_state.alreadySentLogon() && m_state.logonTimedOut() )
      {
        ...
        // cooper {
        //disconnect();
        m_errorCode.set(sec_Timedout_Waiting_For_Logon_Response);
        disconnect( std::string("Timed out waiting for logon response") );
        // } cooper
      }
      return ;
    }
    ...
    if ( m_state.logoutTimedOut() )
    {
      ...
      // cooper {
      //disconnect();
      m_errorCode.set(sec_Timedout_Waiting_For_Logout_Response);
      disconnect( std::string("Timed out waiting for logout response") );
      // } cooper
    }
    ...
    if ( m_state.timedOut() )
    {
      ...
      // cooper {
      //disconnect();
      m_errorCode.set(sec_Timedout_Waiting_For_Heartbeat);
      disconnect( std::string("Timed out waiting for heartbeat") );
      // } cooper
    }
    else
    {
...
{ QF_STACK_PUSH(Session::nextLogon)
  ...
  if( m_state.shouldSendLogon() && !m_state.receivedReset() )
  {
    ...
    // cooper {
    //disconnect();
    m_errorCode.set(sec_Received_Logon_Response_Before_Sending_Request);
    disconnect( std::string("Received logon response before sending request") );
    // } cooper
    return ;
  }
...
{ QF_STACK_PUSH(Session::nextLogout)
  ...
  {
    // cooper {
    if (m_state.alreadySentLogon() && !m_state.receivedLogon())
    {
      m_errorCode.set(sec_Logon_Reject);
    }
    // } cooper
  ...
  if (logout.findField(text))  disconnect( text.getValue() );
  else                         disconnect( std::string()   );
  // } cooper
...
{ QF_STACK_PUSH(Session::disconnect)
  ...
  if ( m_pResponder )
  {
    ...
    char logMsg[512];
    strcpy(logMsg, "Disconnecting");
    if (reason.empty() == false) {
      strcat(logMsg, "; ");
      strcat(logMsg, reason.c_str());
    }
    m_state.onEvent( std::string(logMsg) );

    m_disconnReason = reason;
    // } cooper
  ...
  if ( m_state.receivedLogon() || m_state.sentLogon() )
  {
    ...
    m_application.onLogout( m_sessionID, reason );
  }
...
{ QF_STACK_PUSH(Session::verify)
  ...
  try
  {
    ...
    {
      // cooper {
      m_errorCode.set(sec_Logon_State_Is_Not_Valid_For_Message);
      // } cooper
  ...
  if ( pMsgType )
    fromCallback( *pMsgType, msg, m_sessionID );
  else
...
{ QF_STACK_PUSH(Session::doTargetTooLow)
  ...
  if ( !possDupFlag )
  {
    ...
    // cooper {
    m_errorCode.set(sec_Target_MsgSeqNum_Too_Low);
    // } cooper
  ...
  catch( InvalidMessage& e )
  {
    ...
    try
    {
      if( identifyType(msg) == MsgType_Logon )
      {
        ...
        //disconnect();
        m_errorCode.set(sec_Logon_Reject);
        m_errorCode.set(sec_Logon_Message_Is_Not_Valid);
        disconnect( std::string("Logon message is not valid") );
        // } cooper
      }
  ...
  catch ( FieldNotFound & e )
  {
    ...
    else
    {
      ...
      if ( header.getField(FIELD::MsgType) == MsgType_Logon )
      {
        ...
        // cooper {
        //disconnect();
        m_errorCode.set(sec_Required_Field_Missing_From_Logon);
        disconnect( std::string("Required field missing from logon") );
        // } cooper
      }
    }
  }
