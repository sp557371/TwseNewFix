...
{
   sec_Logon_Reject,
   sec_Logon_State_Is_Not_Valid_For_Message,
   sec_Timedout_Waiting_For_Logon_Response,
   sec_Timedout_Waiting_For_Logout_Response,
   sec_Timedout_Waiting_For_Heartbeat,
   sec_Received_Logon_Response_Before_Sending_Request,
   sec_Logon_Message_Is_Not_Valid,
   sec_Required_Field_Missing_From_Logon,
   sec_Target_MsgSeqNum_Too_Low,
   sec_Total
};
typedef std::bitset<sec_Total>  TSesErrCode;
// } cooper
...
public:
  ...
  Session( Application&, MessageStore*,
           const SessionID&,
           const DataDictionary&,
           const SessionTime&,
           int heartBtInt, Log*
  ...
  void reset() throw( IOException ) 
  { generateLogout(); disconnect( std::string() ); m_state.reset(); }
  // } cooper
  ...
  const TSesErrCode& getSesErrCode    ()  { return m_errorCode;     }
  const std::string& getDisconnReason ()  { return m_disconnReason; }
  // } cooper
...
private:
  ...
  MessageStore*      m_pMessageStore;
  Log*               m_pLog;
  TSesErrCode        m_errorCode;
