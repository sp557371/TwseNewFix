...
public:
  ...
  virtual void onLogout( const SessionID&, const std::string& ) = 0;
...
public:
  ...
  void onLogout( const SessionID& sessionID, const std::string& reason )
  { Locker l( m_mutex ); app().onLogout( sessionID, reason ); }
  // } cooper
...
class NullApplication : public Application
{
  ...
  void onLogout( const SessionID&, const std::string& ) {}
