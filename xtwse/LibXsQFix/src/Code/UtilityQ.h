...
#include <time.h>
#include <stdio.h>
#include <naltypes.h>   // ADDRESS, MAX_NAME_SIZE
#include <utility.h>
//#include "C:\Program Files\Borland\CBuilder5\Include\utility.h"
...
#ifdef _MSC_VER
K_fn(void) file_mkdir( const char* path );
#else
K_fn(void) file_mkdir( const char* path );
#endif
K_fn(FILE*)       file_fopen( const char* path, const char* mode );
K_fn(void)        file_unlink( const char* path );
K_fn(std::string) file_appendpath( const std::string& path, const std::string& file );
