...
public:
  ...
  tm getTmUtc() const 
  {
    ...
    // cooper {
    //   為了讓 BCB 在 compile 時不會有下列 warning
    //      [C++ Warning] FieldTypes.h(199): W8058 Cannot create pre-compiled header: initialized data in header
    //tm result = { 0 };
    tm result;
...
public:
  ...
  // cooper {
  //     因為不想在多生出一些 class, 所以借用 UtcTimeStamp 來當作 LocalTimeStamp
  //UtcTimeStamp()
  //: DateTime( DateTime::now() ) {}
  UtcTimeStamp( bool isUseLocal = false )
  : DateTime( (!isUseLocal ? DateTime::now() : DateTime::nowLocal()) ) {}
  // } cooper
