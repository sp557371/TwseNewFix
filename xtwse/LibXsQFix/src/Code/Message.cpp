...
// cooper {
std::string& Message::toString1( std::string& str, 
                                 int beginStringField,
                                 int bodyLengthField, 
                                 int checkSumField ) const
{ QF_STACK_PUSH(Message::toString1)

  int length = bodyLength( beginStringField, bodyLengthField, checkSumField );
  m_header.setField( IntField(bodyLengthField, length) );
  m_trailer.setField( CheckSumField(checkSumField, checkSum(checkSumField)) );

  // 讓 string 一次把記憶體建好,不用在重新 alloc
  str.reserve( length + 64 );
  
  m_header.calculateString1 ( str );
  FieldMap::calculateString1( str );
  m_trailer.calculateString1( str );
...
FieldBase Message::extractField ( const std::string& string, std::string::size_type& pos,
...
int Message::getVersion () const
{
   ...
   if (m_header.findField(beginString) == false)
      return 0;

   const char* data = beginString.getString().c_str();
   
        if (strcmp(data, BeginString_FIX42) == 0)  return 42;
   else if (strcmp(data, BeginString_FIX44) == 0)  return 44;
   else if (strcmp(data, BeginString_FIX43) == 0)  return 43;
   else if (strcmp(data, BeginString_FIX41) == 0)  return 41;
   else if (strcmp(data, BeginString_FIX40) == 0)  return 40;
