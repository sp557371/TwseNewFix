...
{
   std::string gConfigPath = GetConfPath();
   std::string gTmpPath = gConfigPath + GMainIni; 
   TKIniFile*   iniFile  = new TKIniFile(gTmpPath.c_str());
   std::string gPath = iniFile->ReadString(GMain, GTmpPath, "");
...
K_mf(std::string)  GetAppiaPath()
{
   ...
   std::string gTmpPath = gConfigPath + GMainIni; 
   TKIniFile*   iniFile  = new TKIniFile(gTmpPath.c_str());
   std::string gAppiaPath     = iniFile->ReadString(GMain, GAppiaPath, gConfigPath);
   delete iniFile;
...
K_mf(bool)  GetAppiaLocked()
{
   ...
   std::string gAppiaPath = GetAppiaPath() + GAppiaIni;
   std::string gTmpPath = GetTmpPath() + GMainIni;
   TKIniFile*   iniFile  = new TKIniFile(gTmpPath.c_str());
   bool  isLocked = iniFile->ReadBool(GAppia, GIsLocked, false);
   delete iniFile;
   return bool(isLocked);   
...
{
   // 20180822 add by k288 for 重覆傳輸F03,避免先前資料殘留(已確認F03只會送隔日要生效的資料，所以可以直接清除F03Map_)
...
{
   // 20180822 add by k288 for 重覆傳輸F05,避免先前資料殘留
   F05Map_->clear();
...
K_mf(void)  TwseFilesHandler::ProcessF06(TMarket& mkt, TFileType& type, TStringV& v)
{
   ...
   std::string gConfigPath = GetConfigPath() + GMainIni;
...
{
   // 20180822 add by k288 for 重覆傳輸F11,避免先前資料殘留(已確認F11只會送隔日要生效的資料，所以可以直接清除F11Map_)
   F11Map_->clear();
...
K_mf(bool)  TwseFilesHandler::SetF01IniFile(SF01Data2& f01, TKIniFile& main, TKIniFile& xTester, TKIniFile& xTwse, std::string& applyDate)
{
   ...
   case est_NewFix:
      ...
      //取得middleware name
      key2  = (f01.Market_[0]=='T'?xTester.ReadString(GSummary, "AppiaMWName", "mTester_T"):xTester.ReadString(GSummary, "AppiaMWName", "mTester_O"));
      key1  = "[" + key2 + "]";
      //key1  = "[" + (f01.Market_[0]=='T'?xTester.ReadString(GSummary, "AppiaMWName", "mTester_T"):xTester.ReadString(GSummary, "AppiaMWName", "mTester_O")) + "]";//"[mTester]";
      ...
      //新增至[Main]
      xTwse.WriteStrings(GAppiaMain, GAConnection, connS);
      xTwse.WriteStrings(GAppiaMain, GMiddleware, key1);
      //新增至[middleware]
      xTwse.WriteStrings(key2, GMidSesID, appiaKey); 
      //新增至[connection]
      xTwse.WriteString(appiaKey, GConFixVer, fixVer); //填入FIX版本
      xTwse.EraseIndent(appiaKey, GNetAddress);
      if(f01.IP1_.empty() == false && f01.IP1_.as_string() != "               ")
         xTwse.WriteStrings(appiaKey, GNetAddress, f01.IP1_.as_string());
      if(f01.IP2_.empty() == false && f01.IP2_.as_string() != "               ")
         xTwse.WriteStrings(appiaKey, GNetAddress, f01.IP2_.as_string());
      xTwse.WriteString(appiaKey, GRemotePort, f01.PVCNo_.as_string()); //[Add]20110523
      xTwse.WriteString(appiaKey, GBrokerFirmID, key); //[T5U3U8UAUXU1U] remote_firm_id=T5380X1
      //20100714 by eric 測試線路亦由AppiaParam.ini取得共用設定
      WriteDefaultAppia(xTwse, appiaKey, false);
      //設定回覆F02需要的額外資訊
      xTester.WriteStrings(GSummary, GSesID, key);
      xTester.EraseSection(key);
      xTester.WriteString(key, GTestNo, f01.TestNo_.as_string());
      xTester.WriteString(key, GRemotePort, f01.PVCNo_.as_string()); 
      xTester.WriteString(key, GAPCode, f01.APCode_.as_string());
      xTester.WriteString(key, GPwd, f01.Passwd_.as_string());
      xTester.WriteString(key, GType, f01.Type_.as_string());
      xTester.WriteString(key, GConFixVer, verSrc==std::string("1")?"42":"44"); //for cooper using
      xTester.WriteString(key, GSourceIP1, f01.IP1_.as_string());
      xTester.WriteString(key, GSourceIP2, f01.IP2_.as_string());
      xTester.WriteString(key, GFWGroup, f01.FWGroup_.as_string());
      xTester.WriteString(key, GAppiaGroup, f01.AppiaGroup_.as_string());
      xTester.WriteString(key, GIP, f01.DestIP_.as_string());
      xTester.WriteString(key, GListPort1, f01.ListPort1_.as_string());
      xTester.WriteString(key, GListPort2, f01.ListPort2_.as_string());
      xTester.WriteString(key, GSeqNo, f01.ApplySeqNo_.as_string());
      xTester.WriteString(key, GApplyDate, applyDate);
      if(Mgr_ = GetExcMgr()) {
...
{
   // 20180822 add by k288 for F01Map_ 沒資料時，不用執行以下程序
   if(F01Map_->size() == 0)
      return true;
   ...
   std::string gTmpPath = tmpPath + GMainIni;
   TKIniFile*   iniFile = new TKIniFile(gTmpPath.c_str());
   // gXTesterPath = iniFile->ReadString(GMain, GTesterPath, tmpPath);
   std::string gXTesterPath = iniFile->ReadString(GMain, GTesterPath, tmpPath);
   
   if(gXTesterPath[gXTesterPath.length()-1]!='/' && gXTesterPath[gXTesterPath.length()-1]!='\\')
      gXTesterPath += "/";
   gXTesterPath += "XTester.ini"; //設定XTester.ini

   TKIniFile*  xTesterIni = new TKIniFile(gXTesterPath.c_str());
   
   std::string  applyDate;
   bool         isOK = true;
   bool         boolV;
   
   std::string gAppiaPath = "";
   for(TF01Map::iterator iter=F01Map_->begin(); iter!=F01Map_->end(); ++iter) {
      ...
      sprintf(buf, "Start to Configure XTester, T=%s, A=%s", xTesterIni->GetFilePath().c_str(), appiaIni->GetFilePath().c_str());
      WriteMsg(GXFT, buf);
      
      applyDate = iter->second.ApplyDate_.as_string(Kway::TKDate::sf_ELong, 0);
      boolV = SetF01IniFile(iter->second.F01Data2_, *iniFile, *xTesterIni, *appiaIni, applyDate);
      if(boolV == false)
         isOK = boolV;
      delete  appiaIni;
   }
   
   // 刪除 twse.ini.appiaGroup 內mTester殘留多餘的線路資料
   TStrings Sess, appiaGroups, midSesIDs;
   int sesCnt = xTesterIni->ReadStrings(GSummary, GSesID, Sess);
   for(int i = 0; i < sesCnt; i++)
   {
      Sess[i] = ConvertTand2Appia(Sess[i]);
   }
   std::string appiaMWName = xTesterIni->ReadString(GSummary, GAppiaMWName, (GetProcMkt() == m_Tse ? "mTester_T" : "mTester_O"));
   int appiaGroupCnt = xTesterIni->ReadStrings(GSummary, GAppiaGroup, appiaGroups);
   for(int i = 0; i < appiaGroupCnt; i++)
   {
      gAppiaPath = GetAppiaPath() + GAppiaIni + "." + appiaGroups[i];  //設定Appia.ini
      sprintf(buf, "檢查 %s 有無殘留多餘線路資料。", gAppiaPath.c_str());
      WriteMsg(GXFT, buf);
      TKIniFile*  xtwseIni = new TKIniFile(gAppiaPath.c_str());
      int midSesIDCnt = xtwseIni->ReadStrings(appiaMWName, GMidSesID, midSesIDs);
      for(int j = 0; j < midSesIDCnt; j++)
      {
         if(find(Sess.begin(), Sess.end(), midSesIDs[j]) == Sess.end())
         {
            sprintf(buf, "XTester.ini 內找不到 SesId=%s 的資料，將 %s 內的資料刪除。", midSesIDs[j].c_str(), xtwseIni->GetFilePath().c_str());
            WriteMsg(GXFT, buf);
            xtwseIni->EraseValue(GAppiaMain, GAConnection, "[" + midSesIDs[j] + "]");
            xtwseIni->EraseSection(midSesIDs[j]);
            xtwseIni->EraseValue(appiaMWName, GMidSesID, midSesIDs[j]); 
            continue;
         }
         std::string sesId = ConvertAppia2Tand(midSesIDs[j]);
         std::string appiaGroup = xTesterIni->ReadString(sesId, GAppiaGroup, "");
         if(appiaGroups[i] != appiaGroup)
         {
            sprintf(buf, "%s 的 AppiaGroup=%s 與 XTester.ini 內 [%s] 的 AppigGroup=%s 不同，將 %s 內的資料刪除。", sesId.c_str(), appiaGroups[i].c_str(), sesId.c_str(), appiaGroup.c_str(), xtwseIni->GetFilePath().c_str());
            WriteMsg(GXFT, buf);
            xtwseIni->EraseValue(GAppiaMain, GAConnection, "[" + midSesIDs[j] + "]");
            xtwseIni->EraseSection(midSesIDs[j]);
            xtwseIni->EraseValue(appiaMWName, GMidSesID, midSesIDs[j]); 
...
// --> 20180806 add by k288 for 測試線路與正式線路發生重覆
   bool result = true;
   TF05Map::iterator iter_f05;
   std::vector<SF10Data2>::iterator iter_f10;
   if(vF10Data2.empty() == false) {
      for(iter_f10 = vF10Data2.begin(); iter_f10!=vF10Data2.end(); ++iter_f10) {
         if(file->IsOpened() == false)
            file->Open(fn.c_str(), TFileMode(fmReadWrite|fmCreatePath|fmTrunc));
         f10 = (*iter_f10).ToString();
         file->Write(f10.c_str());
         
         SKey4Prod  key4F05;
         key4F05.Type_ = iter_f10->Type_;
         key4F05.SesID_.Market_.assign(market);
         key4F05.SesID_.Broker_.assign(iter_f10->Broker_.as_string());
         key4F05.SesID_.PVCID_ = iter_f10->PVCID_;

         iter_f05 = F05Map_->find(key4F05);
         if(iter_f05 == F05Map_->end())
         {
            result = false;
            sprintf(buf, "F10 內 FixSes = %s ,在F05中找不到對應的資料", key4F05.SesID_.as_string().c_str());
            if(Mgr_ = GetExcMgr()) {
   ...
   for(iter_f05 = F05Map_->begin(); iter_f05 != F05Map_->end(); iter_f05++)
   {
      bool isFinded = false;
      for(iter_f10 = vF10Data2.begin(); iter_f10!=vF10Data2.end(); ++iter_f10)
      {
      ...
      {
         result = false;
         sprintf(buf, "F05 內 FixSes = %s ,在F10中找不到對應的資料", iter_f05->first.SesID_.as_string().c_str());
         if(Mgr_ = GetExcMgr()) {
            Mgr_->Alert("XF1051", "F10", buf);
         }
         WriteMsg(GXFT, buf);
   ...
   {
      if(Mgr_ = GetExcMgr()) {
         Mgr_->Alert("XF1053", "F10", "F10與F05比對完成");
      }
      using namespace Kway::Tw::Bx::FT;
      TFTNewJob  job;
      job.SeqNo_.assign0();
