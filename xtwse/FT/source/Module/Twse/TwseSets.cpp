...
K_mf(bool) TTwseSetting::GetULCmd(const std::string& fname, const std::string& fdst,
   std::string& cmd, std::string& error) const
{
   std::string path  = GetSetting(fname==std::string("F02")?dsi_F02Path:dsi_F10Path);
   char buf[256];
  ...
  if(path .empty()) { error = SetNameStr[(fname==std::string("F02")?dsi_F02Path:dsi_F10Path)-1]; return false; }
  char buffer[256];
